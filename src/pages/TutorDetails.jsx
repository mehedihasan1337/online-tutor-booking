...
const TutorDetails = () => {
  ...
  const handleSubmit = async e=> {
    ...
    return (
        <div className=' lg:w-8/12 md:w-8/12 mx-auto pt-24 font-Ancizar '>
            <h2 className='text-3xl text-center mb-3 font-bold '>  Tutor Details   </h2>
            <div>
                <div className='  shadow-2xl   '>
                    <div className='flex justify-between pt-3 lg:w-8/12 md:w-8/12 p-2  mx-auto '>
                        ...
                        <div className='flex  gap-2 md:gap-3 lg:gap-3'>
                            <h3 className='font-semibold font-Oswald flex items-center gap-1 text-sm '> <FaStar className='w-5 text-lg text-black dark:text-white' />
                                {review}</h3>
                    ...
                    <hr className='w-8/12  mx-auto font-Ancizar'  />
                    ...
                    <div className=' pl-10 pr-2'>
                        <div className=" pr-2 py-2">
                           ...
                           <span className='font-Oswald'> Language :</span> {language}</p>
                                ...
                                <p className='flex items-center gap-1 text-lg font-semibold'><MdEmail className='text-black dark:text-white ' />
                                <span className='font-Oswald'>  Tutor Email : </span> {buyer?.email}</p>
                                <p className='flex items-center gap-1 text-lg font-semibold'><MdEmail className='text-black dark:text-white ' />
                                <span className='font-Oswald'>  Your Email:</span> {user?.email}</p>
                           
                                <h3 className= 'flex items-center gap-1 font-semibold font-Oswald  text-lg '> <MdOutlinePriceChange />  Pirce : <span className='text-black dark:text-white '> {Price?.price}</span>  {Price?.category} </h3>
                                 <h2 className='font-Oswald font-bold text-xl text-blue-500'>Description</h2>
                                <p className='text-lg font-semibold'>{description}</p>

                                       <button onClick={handleSubmit} className=' px-3 border-b-2 dark:border-white border-black rounded-lg mt-2 text-blue-500 font-Ancizar btn-outline'>Book </button>
                               <div className='flex justify-end '>
                               <Link to={"/findTutors/language"} className=" flex  p-2 rounded-lg bg-blue-500 font-Roboto text-black font-bold hover:bg-blue-600 hover:text-white hover:p-2  "> <GoArrowLeft className="text-2xl"/>Go to category</Link>
